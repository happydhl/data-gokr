...
from tkinter import * # __all__ 따로 정의 해야 하기에 밑에서 sub 함수 또 정의 해 줘야함(?)
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요",\
        ...
        initialdir=r"C:\Users\UserK\Desktop\pythonworkspace") 
    # 최초에 C:/ 경로를 보여줌
...
def browse_dest_path():
    ...
    if folder_selected == '':  #  == Nnoe 이면 안됨 찾아보기 취소 시 문제 됨
        return
...
def merge_image():
    ...
    #[ (10,10), [20,20), (30,30)]]
    #widths , heights = zip(*(x.size for x in images))  #zip 이용한 계산
    widths , heights = zip(*(image_sizes))  #zip 이용한 계산
    ...
    for idx, img in enumerate(images):
        #width 가 원본유지 가아닐 때에는 이미지 크기 조정
        if img_width > -1:
            img = img.resize(image_sizes[idx])


        result_img.paste(img,(0, y_offset))
        y_offset += (img.size[1] + img_space)# height 값 만큼 더 해줌 + 사용자가 지정한 간격
...
def start():
    ...
    if len(txt_dest_path.get()) == 0:
        msgbox("굥고", "저장 경로를 선택 하세요")
        return
    # 이미지 통합 작업
    merge_image() # 길어지므로 위에서 정의 def
