...
# 화면 타이틀 설정
pygame.display.set_caption("이덕행 pang") # 게임 이름
...
current_path = os.path.dirname(__file__) #현대 파일의 위치 반환
...
weapon_width = weapon_size[0] #무기 중안에서 나오니 width 만 필요
#무기는 한번에 여러발 발사 가능
weapons = []
#무기 이동 속도는 캐릭터 보다 빠르게
weapon_speed = 10
...
while running:
    ...
    #무기 위치 조정
    # 100, 200 -> 180, 160, 140 Y 죄표는 줄어든다
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        else:  # 그외에는 속도를 증가  포물선에서 - 속도에서 천천히 변화, 정점에서 조금씩 빠르게 속도
            ball_val["to_y"] += 0.5
    ...
    for ball_idx, ball_val in enumerate(balls):
            ...
            for weapon_idx, weapon_val in enumerate(weapons):
                ...
                if weapon_rect.colliderect(ball_rect):
                    ...
                    if ball_img_idx < 3:
                        ...
                        balls.append({
                            ...
                            "to_x" : -3, #x축 이동방향, -3이면 왼쪽으로, 3이면 오른쪽으로
                            "to_y": -6, # y축 이동 방향
                            "init_spd_y": ball_speed_y[ball_img_idx + 1]}) # y 속도 변경
                ...
                continue # 안쪽 for 문 조건이 맞지 않으면 continue.밖 for 문은 계속 수행
    ...
    for weapon_x_pos , weapon_y_pos in weapons: #그리는 순서 밑에서 위로 조정
        screen.blit(weapon, (weapon_x_pos ,weapon_y_pos)) 

    for indx, val in enumerate(balls):
